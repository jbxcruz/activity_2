...
const roomWalls = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({
        color: '#ac8e82', // Same as house walls, can use textures too
...
roomWalls.geometry.setAttribute(
    'uv2',
    new THREE.Float32BufferAttribute(roomWalls.geometry.attributes.uv.array, 2)
);
roomWalls.position.y = 1.5; // Center the room walls on the floor
room.add(roomWalls);

// Room Roof (simple pyramid)
const roomRoof = new THREE.Mesh(
    new THREE.ConeGeometry(2.5, 1, 4), // radius, height, segments
    new THREE.MeshStandardMaterial({ color: '#F5F5DC' }) // Same as house roof
);
roomRoof.rotation.y = Math.PI * 0.25; // Align the roof with the walls
roomRoof.position.y = 3.5; // Place roof on top of the walls
room.add(roomRoof);

// Door for the room
const roomDoor = new THREE.Mesh(
    new THREE.PlaneGeometry(1, 2), // width, height
    new THREE.MeshStandardMaterial({
        color: '#aa7b7b', // Same as the house door
...
roomDoor.geometry.setAttribute(
    'uv2',
    new THREE.Float32BufferAttribute(roomDoor.geometry.attributes.uv.array, 2)
);
roomDoor.position.set(1.5, 0, 1.51); // Center door at front of the room
room.add(roomDoor);

// Position the entire room to the right of the house
room.position.set(4, 0, 0); // Adjust X to place it beside the house
